    ...
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith('## fragmentCount'):
                ...
                # Extract individual fragment counts from the data section
                fields = line.strip().split('\t')
                if len(fields) > 2:  # Ensure there's enough fields
                    individual_fragment_counts.append(int(fields[2]))  # The third field is fragmentCount
...
def merge_files(mapstat_file_path, res_file_path, output_file_path):
    """Merges .mapstat and .res files into a combined CSV file with individual and total fragmentCounts and calculates AMR abundance."""
    ...
    if len(individual_fragment_counts) == len(res_df):
        res_df['FragmentCount'] = individual_fragment_counts  # Individual fragment counts
        res_df['TotalFragmentCount'] = total_fragment_count  # Total fragment count for all rows
        # Calculate AMR abundance for each row
        res_df['AMRabundance'] = res_df.apply(lambda row: calculate_amr_abundance(row['FragmentCount'], total_fragment_count, row['Template_length']), axis=1)
    else:
        raise ValueError("Mismatch in the number of rows between .mapstat data and .res file entries.")

    # Save the combined DataFrame to a new CSV file
    res_df.to_csv(output_file_path, index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge .mapstat and .res files with individual and total fragment counts and calculate AMR abundance.")
